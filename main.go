	...
	for _, event := range events {
		if event.Type == linebot.EventTypeMessage {
			...
			case *linebot.TextMessage:
				...
				if strings.Contains(message.Text,"??") {bot.ReplyMessage(event.ReplyToken, linebot.NewImageMessage("http://www.cnidea.net/toutiao/u/20160812/162241647425005301640.jpg")).Do()}
			}
		}
	}
}
